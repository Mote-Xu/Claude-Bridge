...
Codex Bridge — 企业微信是 Codex 会话集群的**异步消息总线**。

mote-home 运行 Gateway（24/7 Node.js），通过 Windows Agent HTTP API 在你 Windows 电脑上执行 Codex。同一套 session store（`%USERPROFILE%\.Codex\`），手机和电脑无缝双向同步。
...
共享 md 文件     = 黑板层（会话间共享知识）
  ├─ AGENTS.md       — 项目架构 + 技术栈（静态，人维护）
  ├─ TASK_BOARD.md   — 谁在做什么、哪些文件被锁
  ├─ BRIDGE_LOG.md   — 每次执行的摘要日志（Gateway 自动追加）
  └─ REQUIREMENTS.md — 需求清单
Bridge          = 协调层（会话间通信、事件路由）
Codex 会话      = 执行层（各司其职的 Worker）
```

企微不只是远程终端——它是会话集群的异步消息总线。不同专长的 Codex 会话各司其职，通过企微总线异步协调，自动干活直到项目完成。
...
```
mote-home:
  /mnt/data/Codex-bridge/            ← Gateway 代码 + SQLite DB
  /etc/systemd/system/Codex-bridge-gateway.service

Caddy + CF Tunnel:
  Codex-tunnel.mote-pal.xyz → Caddy :80 → Gateway :8933
  (走主 Tunnel 87fc0324)

Windows (Mote-Office):
  Codex-Bridge Agent: agent/index.js  (Express :9877, 后台静默运行 + VBS 守护)
  Codex:  C:\Users\Mote\AppData\Roaming\npm\Codex.cmd
...
| 目标 | 命令 |
|------|------|
| Gateway 日志 | `ssh mote@100.118.10.0 'journalctl -u Codex-bridge-gateway -f'` |
| Gateway 重启 | `ssh mote@100.118.10.0 'sudo systemctl restart Codex-bridge-gateway'` |
| Gateway 健康 | `curl https://Codex-tunnel.mote-pal.xyz/health` |
...
### 已工作
- 企业微信消息接收/回复
- Windows Agent HTTP 执行 Codex（纯 HTTP，无 SSH）
- `@会话名` 创建/续接会话
- `Codex --resume` 跨消息续接（含 `cd /d` CWD 修复）
...
- CAST_OF_SESSIONS.md 会话角色名册（项目根，gitignore）— 分清谁是主线 worker / 谁是留档 auditor / 谁是墓碑 retired
  - 交互会话：全局 SessionStart hook（`~/.Codex/hooks/cast-of-sessions.js`）建文件 + 提醒会话自登记（源=交互）
  - Bridge 会话：Agent `run-Codex` 汇合点 `upsertCastBridge()` 机械登记（源=🌉 Bridge），不靠 hook 在 pipe 模式触发
...
- 会话启动**按需感知**：
  - Level 0（默认）：只读 AGENTS.md + TASK_BOARD.md
  - Level 1（用户问进度）：读 BRIDGE_LOG.md
  - Level 2（debug/冲突）：读原始事件
- AGENTS.md 加行为规则：改文件前先 `cat TASK_BOARD.md`，迷茫时 `cat BRIDGE_LOG.md`
...
- 任意 Codex 会话之间可以通过 Bridge 互相通信
- 企微变成"消息总线"，不同专长的 Codex 会话各司其职、异步协调
- 技术上只需给 Codex 输出加路由约定（如 `@bridge:send <会话名> <消息>`）
...
~/.Codex/hooks/cast-of-sessions.js — 全局 SessionStart hook（建名册 + 提醒交互会话自登记）
```
...
|------|------|
| `GET /api/health` | 在线检测 |
| `POST /api/discover` | 扫描 `.Codex\projects\`，读 JSONL 取 cwd，按最近活跃排序 |
| `POST /api/list-sessions` | 列 JSONL 文件，含 aiTitle 摘要 |
| `POST /api/run-Codex` | 本地 pipe 调 `Codex.cmd --resume` |
...
- Gateway 放 `/mnt/data/Codex-bridge/`，不放 `/home/mote/`
- Agent 放 Windows 任意位置，`wscript start-hidden.vbs` 开机自启
- 部署变更后提交 `/mnt/data/infra/` git 仓库（infra 配置）
- 本项目只做 Codex Bridge，不动 mote-home 其他服务
- Codex 是唯一 AI 层，Gateway 和 Agent 都不推理
