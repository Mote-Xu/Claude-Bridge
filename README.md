...
Claude Bridge 让你在**手机上通过企业微信**与 Windows 电脑上的 **Claude Code 会话**对话。

电脑上用 VS Code 创建 Claude 会话，出门后用手机企微续接同一个会话，上下文完整保留。回到家打开 VS Code，一切都在——就像你从未离开过。

更深一层：企微成了**多个 Claude 会话的异步消息总线**。你（人）在中间协调，不同专长的会话各司其职、互相通信。
...
Agent 监听 `0.0.0.0:9877`，Gatway 通过 Tailscale IP (`100.80.205.79`) 连接。
